...
pub fn init_logging(config: &LoggingConfig) {
    let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        // release 构建启用了 release_max_level_info，debug/trace 会被优化掉。
        // 若配置文件误写为 debug/trace，将其降级为 info 以避免 tracing-subscriber
        // 的 "static max level is info" warning；其余级别（error/warn/info）保持原样。
        let root_level = if cfg!(debug_assertions) {
            config.level.clone()
        } else {
            match config.level.as_str() {
                "debug" | "trace" => "info".into(),
